...
interface Article {
  id: string; title_nl: string; content_nl: string; published_at: string; category: string
}
...
export default function EditArticleForm({ article }: { article: Article }) {
  ...
  const [title, setTitle] = useState(article.title_nl)
  const [content, setContent] = useState(article.content_nl)
  const [date, setDate] = useState(article.published_at.slice(0, 10))
  ...
  return (
    <div className="min-h-screen pb-10">
      <header className="bg-white border-b border-stone-200 px-4 py-3.5 flex items-center gap-3 sticky top-0 z-10">
        <Link href={`/admin/articles/${article.category}`} className="p-1 -ml-1 text-stone-400"><ArrowLeft size={20} /></Link>
        <h1 className="font-bold text-stone-900 flex-1 truncate">{article.title_nl}</h1>
