...
export default async function RecensiesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 py-12">
      ...
      <h1 className="font-playfair text-4xl font-bold text-forest-900 mb-10">Recensies</h1>
          ...
          {articles.map(a => (
            <article key={a.id} className="py-6">
              ...
              <h2 className="font-playfair text-xl font-semibold text-forest-900 mt-1 mb-2">
                <Link href={localePath(locale, `/over-mij/recensies/${a.id}`)} className="hover:text-ochre-500 transition-colors">
                  {t(a)}
                </Link>
              </h2>
              {a.content_nl && (
              ...
              <Link href={localePath(locale, `/over-mij/recensies/${a.id}`)} className="text-xs text-ochre-500 hover:underline mt-3 inline-block">
