...
export default async function TentoonstellingDetailPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  const { locale, id } = await params
  const supabase = await createClient()
  const { data: article } = await supabase.from('articles').select('*').eq('id', id).eq('category', 'tentoonstellingen').single()
  if (!article) notFound()
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 py-12">
      <Link href={localePath(locale, '/over-mij/tentoonstellingen')} className="text-xs text-ochre-500 hover:underline uppercase tracking-widest mb-6 block">← Tentoonstellingen</Link>
